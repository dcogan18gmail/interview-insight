...
must_haves:
  truths:
    ...
    - 'Sidebar shows project list sorted by most recently updated with interviewee/subject name (falling back to project name), short-form date, and original language'
...
- Each project entry is a minimal clickable stub showing: interviewee name (falling back to project name per LOCKED decision), short-form date, and original language
- Selected project is visually highlighted
- Click on project entry navigates to `/project/:id`
- **NOTE:** This is intentionally a minimal stub. Plan 05-02 replaces this inline rendering with the full `ProjectEntry` component that adds double-click rename and three-dot delete menu.
...
export default function Sidebar() {
  ...
  return (
    ...
    >
      ...
      {/* Project list -- minimal stub; Plan 05-02 replaces with ProjectEntry */}
      {!collapsed && (
        <nav className="flex-1 overflow-y-auto py-2" aria-label="Project list">
          {sortedProjects.map((project) => {
            ...
            // Per LOCKED decision: show interviewee name, fall back to project name
            const displayName = project.interviewee || project.name;
            return (
              <button
                key={project.id}
...
3. Run `npm run lint` -- no lint errors.
   ...
   <done>
   3-panel layout renders correctly: collapsible sidebar with sorted project list on left (using minimal stub rendering with `project.interviewee || project.name` as primary label -- Plan 05-02 replaces with full ProjectEntry), center content area with route-dependent content, right metadata panel for selected projects. Routing updated with nested DashboardLayout. Layout.tsx updated to support full-width content.
   </done>
   </task>
