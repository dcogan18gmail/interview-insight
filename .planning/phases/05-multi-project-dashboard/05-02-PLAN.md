...
must_haves:
  ...
  artifacts:
    - path: 'src/features/dashboard/components/ProjectEntry.tsx'
      provides: 'Sidebar project entry displaying project.interviewee || project.name, with inline rename (double-click) and three-dot menu'
...
- The primary label MUST display `project.interviewee || project.name` per LOCKED decision (sidebar entries show interviewee/subject name). When the interviewee field is set, it takes precedence; otherwise fall back to the project name.

**Inline rename (double-click to edit):**

- Track local `editing` state (boolean) and `draft` state (string).
- On double-click of the project name: set `editing = true`, `draft = project.interviewee || project.name` (pre-fill with what's displayed).
...
- Enter key: trim draft, validate non-empty, call `updateProject({...project, name: trimmed})` from `useProjects()`, exit editing. (Note: rename always updates `project.name` regardless of whether interviewee was being displayed.)
- Escape key: revert `draft` to `project.interviewee || project.name`, exit editing.
...
Replace the minimal stub project list rendering (from Plan 05-01) with `<ProjectEntry>` components:
