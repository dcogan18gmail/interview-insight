...
const FileUpload: React.FC<FileUploadProps> = ({ onFileSelected, disabled }) => {
  ...
  const processFile = useCallback((file: File) => {
    ...
    if (sizeMB > MAX_SIZE_MB) {
      setError(`File is too large (${sizeMB.toFixed(1)}MB). For this demo, please use files under ${MAX_SIZE_MB}MB.`);
      setLoading(false);
      return;
    }
    ...
    <div className="w-full max-w-2xl mx-auto mb-8">
      <div
        ...
        className={`
          ...
          ${disabled || loading ? 'opacity-50 cursor-not-allowed bg-gray-50 border-gray-300' :
            dragActive ? 'border-indigo-500 bg-indigo-50 scale-[1.02]' : 'border-gray-300 bg-white hover:border-indigo-400 hover:bg-gray-50'}
        `}
      >
        <input
          type="file"
        ...
        <div className="flex flex-col items-center justify-center pt-5 pb-6 pointer-events-none">
          {loading ? (
            ...
            <>
              <svg
                className={`w-12 h-12 mb-4 ${dragActive ? 'text-indigo-500' : 'text-gray-400'}`}
              ...
              <p className="text-xs text-gray-400">MP3, WAV, MP4, MOV (MAX {MAX_SIZE_MB}MB)</p>
            </>
          )}
        </div>
      </div>
      {error && (
        <div className="mt-2 p-3 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg flex items-center animate-pulse">
